...
hours = float(input())  # Can be input as a decimal if parked partial hours
hours = ceil(hours)  # makes partial hour rounded up to next full hour
if hours > 0:  # if you entered a positive number for hours parked it will use this part
    if hours <= 2:
        fee = 4
...
if hours < 0:  # if you entered a negative value for hours parked because of lost ticket this part will be used
    hours = -(hours)  # this makes hours back to a positive number so you can reuse code from above
